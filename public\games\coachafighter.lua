...
local function AddDiscordButton(Tab)
	...
	DiscordGroup:AddButton({
		Text = "Join Ecco Discord: discord.gg/hN9QpA3HA",
		Func = copyDiscord,
	})
	DiscordGroup:AddButton({
		Text = "Ecco Hub V3 | Premium Keyless",
		Func = copyDiscord,
	})
end
...
local AdGroup = Tabs.Info:AddLeftGroupbox("Ecco Hub V3", "sparkles")
...
SaveManager:SetFolder("EccoHubV3/coach-a-fighter")
...
task.spawn(function()
	while not Library.Unloaded do
		...
		rerollBusy = false
		if not ok then
			warn("[Ecco Hub V3] Reroll loop:", err)
		end
	end
end)
...
task.spawn(function()
	while not Library.Unloaded do
		...
		if not ok then
			warn("[Ecco Hub V3] Training loop:", err)
		end
	end
end)
...
task.spawn(function()
	...
	while not Library.Unloaded do
		...
		sparBusy = false
		if not ok then
			warn("[Ecco Hub V3] Spar loop:", err)
		end
	end
end)
...
task.spawn(function()
	while not Library.Unloaded do
		...
		if not ok then
			warn("[Ecco Hub V3] Boxer loop:", err)
		end
	end
end)
