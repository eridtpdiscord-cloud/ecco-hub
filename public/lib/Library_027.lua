...
do
    ...
    function Library:SetWatermark(Text: string)
        -- warn suppressed by Ecco Hub V3
...
function Library:CreateWindow(WindowInfo)
        ...
        if WindowInfo.Icon then
            ...
            pcall(function()
                ...
                if getcustomasset and isfile and isfile("ecco_symbol.png") then
                    ...
                    WindowIcon.ImageRectSize = Vector2.zero
                    WindowIcon.ImageRectOffset = Vector2.zero
                end
            end)
        else
    ...
    -- [[ ECCO HUB V3 AUTOMATIC TOP INFO TAB ]]
    task.spawn(function()
        task.wait(0.05)
        pcall(function()
            local infoTab = Window:AddTab({
                Name = "Info",
                Icon = "info",
                Description = "Ecco Hub V3 Official Information",
                Order = -9999
            })
            if infoTab then
                ...
                local detailsBox = infoTab:AddLeftGroupbox("Game Information")
                detailsBox:AddLabel({ Text = "Game: " .. tostring(game.Name or "Supported Game") })
                detailsBox:AddLabel({ Text = "Place ID: " .. tostring(game.PlaceId) })
                detailsBox:AddLabel({ Text = "Universe ID: " .. tostring(game.GameId) })
                detailsBox:AddLabel({ Text = "Job ID: " .. tostring(game.JobId):sub(1, 16) .. "..." })

                local socialsBox = infoTab:AddRightGroupbox("Ecco Hub Socials")
                socialsBox:AddLabel({ Text = "Discord: discord.gg/hN9QpA3HA" })
                socialsBox:AddButton({
                    ...
                    Func = function()
                        ...
                        Library:Notify({ Title = "Ecco Hub V3", Description = "Discord link copied to clipboard!", Time = 3 })
                    end
                })
                socialsBox:AddDivider()
                socialsBox:AddLabel({ Text = "TikTok: @_ecc00_" })
                socialsBox:AddButton({
                    Text = "Copy TikTok Link",
                    Func = function()
                        local cb = setclipboard or toclipboard or (Clipboard and Clipboard.set)
                        if cb then cb("https://www.tiktok.com/@_ecc00_?is_from_webapp=1&sender_device=pc") end
                        Library:Notify({ Title = "Ecco Hub V3", Description = "TikTok profile copied to clipboard!", Time = 3 })
                    end
                })

                local playerBox = infoTab:AddLeftGroupbox("Player Profile")
                playerBox:AddLabel({ Text = "User: " .. tostring(lp.Name) })
                playerBox:AddLabel({ Text = "Display: " .. tostring(lp.DisplayName) })
                playerBox:AddLabel({ Text = "User ID: " .. tostring(lp.UserId) })
                playerBox:AddLabel({ Text = "Account Age: " .. tostring(lp.AccountAge) .. " days" })

                local serverBox = infoTab:AddRightGroupbox("Server Controls")
                serverBox:AddButton({
                    Text = "Rejoin Game",
                    Func = function()
